...
parse_nmea(char *line, char *message)
{
  ...
  if (pgn->valueint == 127250) {
	...
	printf("  DEVICES %s => %s\n", c_newstate.devices[x].unique_number, c_newstate.devices[x].model_id);
      }
    }
...
cJSON          *
build_diff(struct nmea_state *old_state, struct nmea_state *new_state, struct claim_state *c_old_state, struct claim_state *c_new_state)
{
  ...
  changes += diff_claim_values(ret, c_old_state->claims, c_new_state->claims, "claims");
  changes += diff_device_values(ret, c_old_state->devices, c_new_state->devices, "devices");
...
int
diff_claim_values(cJSON * root, struct claim *old_arr, struct claim *new_arr, char *fieldname)
{
  int		  diff = 0;
  ...
  for (int n = 0; n < MAXCLAIMS; n++) {
    ...
    for (int o = 0; o < MAXCLAIMS; o++) {
      if (new_arr[n].src == old_arr[o].src && !strcmp(new_arr[n].unique_number, old_arr[o].unique_number)) {
	found = 1;
      }
    }
...
int
diff_device_values(cJSON * root, struct device *old_arr, struct device *new_arr, char *fieldname)
{
  int		  diff = 0;
  cJSON          *src = cJSON_CreateObject();
  ...
  for (int n = 0; n < MAXDEVICES; n++) {
    ...
    if (!found) {
      ...
      cJSON_AddStringToObject(v, "model_version", new_arr[n].model_version);
...
int
diff_values(cJSON * root, struct nmea_value *old_arr, struct nmea_value *new_arr, char *fieldname)
{
  int		  diff = 0;
  cJSON          *src = cJSON_CreateObject();
...
update_nmea_claim(cJSON * json, struct claim *arr)
{
  ...
  if (fields) {
    cJSON          *value = cJSON_GetObjectItemCaseSensitive(fields, "Unique Number");
    if (value) {
      char	      unique_number[DLENGTH];
...
update_nmea_device(cJSON * json, char *unique_number, struct device *arr)
{
  ...
  if (fields) {
    ...
    cJSON          *software_version_code = cJSON_GetObjectItemCaseSensitive(fields, "Software Version Code");
    cJSON          *model_version = cJSON_GetObjectItemCaseSensitive(fields, "Model Version");
	...
	if (arr[x].isactive == 0) {
	  ...
	  strcpy(arr[x].unique_number, unique_number);
	  if (cJSON_IsString(model_id) && model_id->valuestring != NULL) {
	    strcpy(arr[x].model_id, model_id->valuestring);
	  }
	  printf("Adding! 2\n");

	  if (cJSON_IsString(model_version) && model_version->valuestring != NULL) {
	    strcpy(arr[x].model_version, model_version->valuestring);
	  }
	  printf("Adding! 2\n");

	  if (cJSON_IsString(software_version_code) && software_version_code->valuestring != NULL) {
	    strcpy(arr[x].software_version_code, software_version_code->valuestring);
	  }
	  printf("Adding! 2\n");

	  if (cJSON_IsString(model_serial_code) && model_serial_code->valuestring != NULL) {
	    strcpy(arr[x].model_serial_code, model_serial_code->valuestring);
	  }
	  printf("Adding! 2\n");
