...
static const char *server_address = "community.nakedsailor.blog", *pro = "charlotte-data";
...
connect_client(lws_sorted_usec_list_t * sul)
{
    ...
    i.path = "/timescaledb/boat/test/data";
    i.host = i.address;
    i.origin = i.address;
		 ...
		 void *user, void *in, size_t len)
{
	...
	/*
	 * if ((*vhd->options & 1) && pmsg && pmsg->final) mco->completed =
	 * 1;
	 */
	...
	(int)lws_ring_get_count_free_elements(mco->ring) > RING_DEPTH - 5) {
...
ws_write(char *buf, int len)
{
    ...
    if (pss) {
	fprintf(stderr, "pss is non-null\n");
    }
    if (!pss->ring) {
	fprintf(stderr, "no ring yet\n");
	return 0;
    }
    ...
    fprintf(stderr, "In ws_write '%s' '%s'\n", buf, amsg.payload + LWS_PRE);

    /*
     * for (int x = 0; x < amsg.len; x++) { fprintf(stderr, "Char %d = %c\n",
     * x, buf[x]); } debug_flag = 1;
     */
    ...
    fprintf(stderr, "Callback on writable \n");
